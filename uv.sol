...
contract UV is Mortal,Token {
  ...
  {
    vouchers.push(Voucher(_id, keccak256(_key), _amount, 0, 0, block.timestamp));
  ...
  function consumeVoucher (uint _index, uint _key) returns (bool success) {
    // if consume
    // modifier
    if (vouchers[_index].consumedAt > 0) {
        throw;
    }
    ...
    transfer (msg.sender, vouchers[_index].amount);

    // event Transfer
    Transfer(owner, msg.sender, vouchers[_index].amount);
    // event Consume
  ...
  /// @notice send `_amount` token to `owner` from `msg.sender`
  /// @param _amount The amount of token to be withdraw
  /// @return Whether the transfer was successful
  function withdraw (uint _amount) onlyOwner returns (bool success) {
